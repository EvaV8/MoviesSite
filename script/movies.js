...
const createNewCard = (filmObj,i) => {
    ...
    // if(filmObj.filmGenres.includes('Action')){
    //     movieCard.innerHTML += `
...
//------------------------------ SEARCH -------------------------
let titleByUser = document.getElementById('titleSearch');
const searchBtn = document.getElementById('searchBtn');

searchBtn.addEventListener('click', ()=>{
    const word = titleByUser.value;
    console.log(word);
    for(let i=0; i<titlesArray.length; i++) {
        if(titlesArray[i].includes(word)){
            let titleUser = titlesArray[i];
            let index = i;
            const apiKey = key;
            getOmdb(titleUser, index, apiKey);
...
function hideOther(){
    const moviesCards = document.getElementById('movie-card-demo');
    moviesCards.classList.add('searchHidden')
}
